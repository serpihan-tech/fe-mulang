...
export default function TahunAjar() {
  ...
  const fetchDataSemester = async (
    ...
    setIsLoading(true);
    try {
      const data = await data_semester(
        limitVal,
        page,
        search,
        sortField,
      ...
      if (Array.isArray(dataArray)) {
        ...
        const formattedData = dataArray.map((item) => ({
          ...
          tanggal_mulai:
            format(new Date(item.dateStart), "dd-MM-yyyy") || "Tidak ada",
          tanggal_selesai:
            format(new Date(item.dateEnd), "dd-MM-yyyy") || "Tidak ada",
          ...
          <div className="w-full ps-2">
            ...
            <div className="flex flex-col justify-end bg-white dark:bg-dark_net-pri rounded-lg my-5">
              ...
              {meta && (
                <PaginationComponent
                  meta={meta}
                  onPageChange={fetchDataSemester}
