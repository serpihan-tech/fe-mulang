...
export default function JadwalHari({ day, schedule }) {
    ...
    return (
        <div className="flex flex-col items-center p-4 rounded-lg min-w-[400px] mx-4">
            <h2 className="text-lg text-pri-main font-semibold mb-2">{day}</h2>
            <div className="w-full space-y-5">
                {schedule.length > 0 ? (
                    schedule
                        .sort((a, b) => moment(a.start_time, "HH:mm:ss").valueOf() - moment(b.start_time, "HH:mm:ss").valueOf())
                        .map((item, index) => {
                            let status = "";
                            let statusColor = "";
